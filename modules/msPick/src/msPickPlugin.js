...
version:1.0
参数介绍：
type:'industry',	// 值为 city/industry/job 根据它控制input的name city/cityCode,数据等
...
define(function (require, exports){
     var cityIndustryJob = require('dataFile').cijData;
    (function($){
        $.fn.msPickPlugin = function(options)
        {
            ...
            switch(msPick.type)
            {
                ...
                msPick.structure ="<div class='ms-pick' id='"+msPick.id+"'>"+
                                    ...
                                    "<div class='tab-con-component tab-con-width' style='display:none'>"+
                                        "<div class='ms-h3-component'>热门城市</div>"+
                                            "<div class='hot-city'>"+
                                            "</div>"+
                                        "<div class='ms-h3-component' >其它省市</div>"+
                                            "<div class='otherAll'>"+
                                            "</div>"+
                                        "</div>"+
                                    "</div>"+
                                "</div>";
                msPick.allData = cityIndustryJob.city[0];
                ...
                for(var i=0;i<msPick.allData.nodes.length;i++)
                {
                    ...
                    {
                        if(msPick.allData.nodes[i].nodes.length==1)
                        {
                            msPick.hotData.nodes.push(msPick.allData.nodes[i].nodes[j]);
                        }
                ...
                msPick._allData.nodes = msPick.allData.nodes.slice(4).concat([]);
                msPick.allData=[];
                msPick.allData=msPick._allData;
                break;
                case 'industry':
                ...
                msPick.structure ="<div class='ms-pick' id='"+msPick.id+"'>"+
                                    ...
                                    "</div>"+
                                "</div>";
                msPick.allData = cityIndustryJob.industry;
                msPick.listData = 
                {
                    "nodes":[]
                };
                ...
                case 'job':
                msPick.id = "msPickJob";
                msPick.structure ="<div class='ms-pick' id='"+msPick.id+"' style='width:570px'>"+
                                    "<div class='w-h-tab-component'>"+
                                        "<ul class='h-tab clear'>"+
                                    ...
                                    "</div>"+
                                    "<div class='c-tab'>"+
                                        "<div class='tab-con-component tab-con-width'>"+
                ...
                for(var i=0;i<cityIndustryJob.job.length;i++)
                {
                    for(var j=0;j<cityIndustryJob.job[i].nodes.length;j++)
                    {
                        msPick.allData.nodes.push(cityIndustryJob.job[i].nodes[j]);
                    }
            ...
            msPick.txt = function()
            {
                ...
                $msTxt.on("keyup",function(event){
                    ...
                    if(!bKey)
                    {
                        ...
                        {
                            str = "<div class='ms-error-component'>您输入的地点不存在，请重新输入</div>"; 
                        }
